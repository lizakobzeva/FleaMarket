...
def create_chat(chat: schemas.ChatCreate, db: Session = Depends(get_db), background_tasks: BackgroundTasks = None):
    """Создать новый чат"""
    db_chat = models.Chat(
        item_id=chat.item_id,
        buyer_id=1,  # пока что заглушка
    ...
    if background_tasks:
        background_tasks.add_task(
            write_log,
            f"Chat created: id={db_chat.id}, item_id={db_chat.item_id}, seller_id={db_chat.seller_id}"
        )
...
def complete_chat(chat_id: int, db: Session = Depends(get_db)):
    chat = db.get(models.Chat, chat_id)
    if not chat:
        raise HTTPException(status_code=404, detail="Chat not found")
